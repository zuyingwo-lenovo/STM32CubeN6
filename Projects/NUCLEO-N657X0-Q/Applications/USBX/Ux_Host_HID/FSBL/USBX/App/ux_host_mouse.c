...
VOID hid_mouse_thread_entry(ULONG thread_input) {
  ...
  while (1) {
    ...
    if ((mouse != NULL) && (mouse->ux_host_class_hid_mouse_state ==
                            (ULONG)UX_HOST_CLASS_INSTANCE_LIVE)) {
      /* Get Mouse position */
      if (ux_host_class_hid_mouse_position_get(mouse, &actual_Pos_x,
                                               &actual_Pos_y) == UX_SUCCESS) {
      ...
      if (ux_host_class_hid_mouse_buttons_get(mouse, &actual_mouse_buttons) ==
          UX_SUCCESS) {
      ...
      if (ux_host_class_hid_mouse_wheel_get(mouse, &actual_mouse_wheel) ==
          UX_SUCCESS) {
        if ((actual_mouse_wheel != old_mouse_wheel) &&
            (actual_mouse_wheel != 0)) {
